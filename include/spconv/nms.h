...
// template <typename DType>
// std::vector<int> soft_rotate_non_max_suppression_cpu(py::array_t<DType> box_corners,
                                                // py::array_t<int> score,
                                                // py::array_t<DType> standup_iou,
                                                // DType thresh,
                                                // DType method) {
//
  // auto box_corners_r = box_corners.template unchecked<3>();
  // auto score_r = score.template unchecked<1>();
  // auto standup_iou_r = standup_iou.template unchecked<2>();
  // std::vector<int> keep;
  // int i, j;
  // float weight, sigma=0.5;
//
  // namespace bg = boost::geometry;
  // typedef bg::model::point<DType, 2, bg::cs::cartesian> point_t;
  // typedef bg::model::polygon<point_t> polygon_t;
  ...
  // DType inter_area, union_area, overlap;
//
  // int inds[ndets] = {0,}, pos = 0, maxpos = 0, maxscore = 0;
//
  // for (int i=0;i<ndets;i++) inds[i] = i;
  // for(int i=0; i<ndets; i++) {
    // maxscore = boxes[i][4];
    // maxpos = i;
//
    ...
    // }
//
    // std::swap(box_corners_r[i][0][0], box_corners_r[maxpos][0][0]);
    // std::swap(box_corners_r[i][1][0], box_corners_r[maxpos][1][0]);
    // std::swap(box_corners_r[i][2][0], box_corners_r[maxpos][2][0]);
    // std::swap(box_corners_r[i][3][0], box_corners_r[maxpos][3][0]);
    // std::swap(box_corners_r[i][0][1], box_corners_r[maxpos][0][1]);
    // std::swap(box_corners_r[i][1][1], box_corners_r[maxpos][1][1]);
    // std::swap(box_corners_r[i][2][1], box_corners_r[maxpos][2][1]);
    // std::swap(box_corners_r[i][3][1], box_corners_r[maxpos][3][1]);
    // inds[i] = inds[maxpos];
//
    // pos = i+1;
    // for (int j = i + 1; j < ndets; ++j) {
      // if (standup_iou_r(inds[i], inds[j]) <= 0.0)
        // continue;
      // try {
        // bg::append(poly,
                   // point_t(box_corners_r(i, 0, 0), box_corners_r(i, 0, 1)));
        ...
        // if (!poly_union.empty()) { // ignore invalid box
          ...
          // if (overlap >= thresh) {
            ...
            // } else if (method == 2) { // gaussian
              // weight = std::exp(-1*overlap*overlap/sigma) ;
            // } else { // original NMS
              // weight = 0;
            // }
            // suppressed_rw(j) = 1;
