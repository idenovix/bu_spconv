...
template <typename DType>
py::array_t<DType>
rbbox_iou_loss(py::array_t<DType> box_corners, py::array_t<DType> qbox_corners) {
  // for iou loss 
  // I fix O(NK) -> O(N) (already allocate target gt box at iou loss part)
  namespace bg = boost::geometry;
  typedef bg::model::point<DType, 2, bg::cs::cartesian> point_t;
  typedef bg::model::polygon<point_t> polygon_t;
  polygon_t poly, qpoly;
  ...
  auto N = box_corners_r.shape(0);
  auto K = qbox_corners_r.shape(0);
  py::array_t<DType> overlaps = zeros<DType>({int(N)});
  auto overlaps_rw = overlaps.template mutable_unchecked<1>();
  if (N == 0 || K == 0) {
    return overlaps;
  }
  for (int n = 0; n < N; ++n) {
